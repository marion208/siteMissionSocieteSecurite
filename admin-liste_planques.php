...
?>
        ...
        <body>
            ...
            <div class="container">
                ...
                <?php
                // Vérifier s'il y a des planques en base de données
                include 'Controller/controller_planque.php';
                $controller = new ControllerPlanque();
                $controller->checkIfPlanqueInDB();
                $nbPlanquesInDB = $_SESSION['nbPlanquesInDB'];

                // Si aucune planque en base de données, on affiche un message associé pour prévenir l'utilisateur
                if ($nbPlanquesInDB == 0) {
                    include 'views/messageNoPlanqueInDB.php';
                }
                // Sinon on importe la liste des planques présentes en base de données
                else {
                    $controller->listOfPlanques();
                }
                ?>
            </div>
