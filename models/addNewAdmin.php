...
if (strstr($mailAdmin, " ")) {
    array_push($listErrors, "L'adresse e-mail renseignée ne doit pas contenir d'espace.");
}
...
if (count($listErrors) == 0) {
    $req = $bdd->prepare('INSERT INTO liste_admins (nomAdmin, prenomAdmin, mailAdmin, motDePasseAdmin, dateDeCreationAdmin) VALUES(?, ?, ?, ?, ?)');
    $req->execute(array($nomAdmin, $firstnameAdmin, $mailAdmin, $passwordAdmin, $dateCreateAdmin));
    ...
    header("Location: http://localhost/admin-add_admin.php");
    exit;
}
