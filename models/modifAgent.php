...
if (count($listErrors) == 0) {
    $req = $bdd->prepare('UPDATE liste_agents SET nomAgent = ?, prenomAgent = ?, dateDeNaissanceAgent = ?, codeDIdentificationAgent = ?, nationaliteAgent = ?, specialitesAgent = ? WHERE idAgent = ?');
    $req->execute(array($nomAgent, $firstnameAgent, $birthDate, $codeIdentity, $nationality, $speciality, $idAgent));
    ...
    header("Location: http://localhost/admin-modif_agent.php?id=" . $idAgent);
    exit;
}
